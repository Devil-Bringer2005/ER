...
Varyings BakedLitForwardPassVertex(Attributes input)
{
    ...
    float3 positionOS = input.positionOS.xyz;
        ...
        CURVEDWORLD_TRANSFORM_VERTEX_AND_NORMAL(positionOS, input.normalOS, input.tangentOS)
    #else
...
half4 BakedLitForwardPassFragment(Varyings input) : SV_Target
{
    ...
    // Decals
#ifdef _DBUFFER
    ApplyDecalToBaseColorAndNormal(
        input.positionCS,
        color,
        inputData.normalWS
    );
    ...
    // Baked lighting
    half4 finalColor = UniversalFragmentBakedLit(
        inputData,
        color,
        alpha,
        normalTS
