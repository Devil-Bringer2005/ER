...
Varyings LitPassVertexSimple(Attributes input)
{
    ...
    output.normalWS    = half4(NormalizeNormalPerVertex(normalInput.normalWS), viewDirWS.x);
    output.tangentWS   = half4(NormalizeNormalPerVertex(normalInput.tangentWS), viewDirWS.y);
    output.bitangentWS = half4(NormalizeNormalPerVertex(normalInput.bitangentWS), viewDirWS.z);
...
// Used for Standard Simple Lit shader
...
#if defined(_DBUFFER)
    InitializeRenderingLayers(input.positionCS);
#endif
